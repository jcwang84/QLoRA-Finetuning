...
def main():
    """
    LoRA fine-tuning with Gemma-3-270M (efficient and capable for instruction following)
    """
    print("🚀 Starting LoRA Fine-tuning with Gemma-3-270M")
    print("="*50)
    
    # Step 1: Load Gemma-3-270M model and tokenizer
    model_name = "google/gemma-3-270m"  # 270M parameters, efficient model for instruction following
    ...
    # Step 4: Configure LoRA for Gemma-3-270M
    print("\n⚙️ Configuring LoRA for Gemma-3-270M...")
    lora_config = LoraConfig(
        r=8,  # Reduced rank for stability
        lora_alpha=16,  # Reduced alpha (should be 2x rank)
    ...
    training_args = TrainingArguments(
        output_dir="./gemma-3-270m-fine-tuned",
        per_device_train_batch_size=1,  # Smaller batch size for stability
        gradient_accumulation_steps=4,  # Reduced effective batch size
    ...
    # Step 8: Save model
    print("\n💾 Saving model...")
    model.save_pretrained("./gemma-3-270m-fine-tuned")
    tokenizer.save_pretrained("./gemma-3-270m-fine-tuned")
    print("✅ Model saved to ./gemma-3-270m-fine-tuned/")
